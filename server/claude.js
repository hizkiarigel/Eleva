...
// Founder feedback (job-hunting quests must always be labora-chain, never a
// bare single feature): the prompt above now says so explicitly, but prompt
// compliance is never guaranteed - same "model proposes, code guarantees"
// backstop philosophy as looksRecoveryThemed/normalizeCompletionType's own
// downgrade chain. If the model still emits standalone "job-match-analysis"
// despite the instruction, upgrade it in place to a labora-chain instead of
// letting a single-feature quest ship. The pairing [job-match-analysis,
// job-application-submit] is always >= laboraChainLib.MIN_STEPS
// (normalizeChain below never strips submit when job-match-analysis is
// present in the same chain - see laboraChain.js), so this can never bounce
// back to a single-feature quest via normalizeLaboraChain's own fallback.
// "job-application-submit" is deliberately NOT touched here - it stays a
// legitimate standalone type (see its own prompt paragraph above for why:
// bundling it with a fresh same-day job-match-analysis risks the auto-skip
// logic blocking a submission that was already qualified from a prior day).
function forceJobHuntingChain(quest) {
  if (!quest || quest.completionType !== "job-match-analysis") return quest;
  quest.completionType = "labora-chain";
  quest.laboraChain = { steps: [{ feature: "job-match-analysis" }, { feature: "job-application-submit" }] };
  return quest;
}
...
// N-Domain extension (22 Agustus): TRAINING is an optional 3rd domain the
// AI can additionally flag (multiDomain.includeTraining) on top of the
// base Recovery+Nutrition pair - picks the matching template, never any
// other combination. includeTraining false/absent produces byte-identical
// output to before this extension (same base template, same 2-key
// featureData/featureState shapes) - fully backward compatible.
function normalizeMultiDomainQuest(result) {
  if (!result?.multiDomain?.use) return result;
  const includeTraining = Boolean(result.multiDomain.includeTraining);
  const t = includeTraining ? questHub.RECOVERY_NUTRITION_TRAINING_TEMPLATE : questHub.RECOVERY_NUTRITION_TEMPLATE;
  const featureData = { RECOVERY: {}, NUTRITION: {} };
  const featureState = { RECOVERY: "NOT_STARTED", NUTRITION: "NOT_STARTED" };
  if (includeTraining) { featureData.TRAINING = {}; featureState.TRAINING = "NOT_STARTED"; }
  result.quest = {
    ...result.quest,
    mode: "quest",
...
async function generateQuest(ctx) {
  ...
  try {
    ...
    forceJobHuntingChain(result.quest);
...
async function processReflection(ctx) {
  ...
  try {
    ...
    const evaluationRules = ctx.laboraChainResult
      ...
      // N-Domain extension: generic over however many features this quest
      // actually has (2 or 3) - was hardcoded to literally "Recovery"/
      // "Nutrition" before, which silently dropped a 3rd (TRAINING) area
      // from the mentor's own context.
      ? `Quest hari ini bertipe MULTI-DOMAIN (Quest Hub — ${Object.keys(ctx.multiDomainResult.featureState).join(" + ")}, semua area harus lengkap sebelum quest ini resolve, dikerjakan urutan bebas): status akhirnya "${ctx.multiDomainResult.status}" — ${Object.entries(ctx.multiDomainResult.featureState).map(([k, v]) => `${k} ${v}`).join(", ")}. Setiap area COMPLETE lewat field yang BENAR-BENAR diisi user (bukti, bukan checklist kosong), sudah divalidasi kode sebelum sampai ke kamu. statDeltas WAJIB diisi wajar HANYA untuk stat "body" — primaryFeature quest ini RECOVERY, jadi growth SEMUA area (termasuk area lain) kredit ke stat itu SAJA, JANGAN split/tambahkan ke stat lain meskipun area lain juga tersentuh, itu double-counting yang harus dihindari. mentorReply: sebut data konkret dari SETIAP area yang ada (tidur/energi/soreness/jenis recovery session dari Recovery; protein/hidrasi/meals dari Nutrition; kalau ada TRAINING, durasi/jarak/effort sesi latihannya) — jangan cuma bahas satu-dua area dan abaikan yang lain.`
      : ctx.nutritionResult
...
module.exports = {
  ...
  normalizeCompletionType, forceJobHuntingChain, fallbackReflection, looksRecoveryThemed, analyzeNutritionPhoto,
